...
void send_status ( proc_t* proc, local_id dst, MessageType status ) {
    //printf("proc %i sending single msg of type %i with curr_time %i \n", proc->id, status, proc->b_state.s_time);
    Message msg = create_message ( proc, status, NULL, 0 );
    //printf("proc %i created single msg of type %i with curr_time %i \n", proc->id, status, proc->b_state.s_time);
