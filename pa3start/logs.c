...
char *create_payload(MessageType type, pr_info * pr)
{
    ...
    {
        case STARTED:
            sprintf(payload, log_started_fmt, pr->b_state.s_time,  pr->id, getpid(), getppid(), pr->b_state.s_balance);
            break;
        case DONE:
            sprintf(payload, log_done_fmt, pr->b_state.s_time,  pr->id, pr->b_state.s_balance);
            break;
        case STOP:
            payload = NULL;
            break;
