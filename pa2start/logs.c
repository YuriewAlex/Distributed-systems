...
int start_logs(){
    pipelog1 = open(pipes_log, O_CREAT | O_RDWR | O_APPEND);
    eventlog1 = open(events_log, O_CREAT | O_RDWR | O_APPEND);
...
void write_receive_done(pr_info * pr){
    char *buffer = malloc(sysconf(_SC_PAGESIZE));
    sprintf(buffer, log_received_all_done_fmt, get_physical_time(),  pr->id);
    write_event(pr, buffer);
}

void write_recieve_started(pr_info * pr){
    char *buffer = malloc(sysconf(_SC_PAGESIZE));
    sprintf(buffer, log_received_all_started_fmt,  get_physical_time(), pr->id);
    write_event(pr, buffer);
}
...
char *create_payload(MessageType type, pr_info * pr)
{
    ...
    {
        case STARTED:
            sprintf(payload, log_started_fmt, get_physical_time(),  pr->id, getpid(), getppid(), pr->b_state.s_balance);
            break;
        case DONE:
            sprintf(payload, log_done_fmt, get_physical_time(),  pr->id, pr->b_state.s_balance);
            break;
        case STOP:
            payload = NULL;
            break;
...
Message create_msg(MessageType type, void* payload, pr_info* pr){
    Message msg;
    ...
    if( payload != NULL ) {
        ...
        case BALANCE_HISTORY:
            msg.s_header.s_payload_len = (pr->b_history.s_history_len) * sizeof(BalanceState) + 
            sizeof(pr->b_history.s_history_len) + 
            sizeof(pr->b_history.s_id);
            break;
        case TRANSFER:
            msg.s_header.s_payload_len = sizeof(TransferOrder);
            break;
        default:
            msg.s_header.s_payload_len = sizeof(payload);
            break;
        }
